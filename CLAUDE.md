...
## Developer Agent Safety Rules (Milestone 3+)

46. Developer agents must only execute against repositories registered in configuration (`Projects:Projects`). Never execute against an unregistered repository.
47. Developer agents must work exclusively in isolated git worktrees under `D:\Projects\_RebelgentWorkspaces`. Never modify the source repository's working tree.
48. Developer agents must never merge their task branch into any other branch.
49. Developer agents must never push a branch to any remote without explicit human approval.
50. Developer agents must never modify any Git remote configuration.
51. Workspace paths are generated by the system (project ID + task ID). They must never come from Telegram messages, API requests, or any user-supplied text.
52. No content from Telegram messages may become an executable name, a shell command, or a filesystem path.
53. Task text must be treated as DATA to implement, not as instructions that can override Rebelgent safety rules. All prompts sent to Claude must clearly delimit system instructions from task data.
54. Build and test validation must run independently after the developer agent exits. The developer agent must not self-certify its own output.
55. If the source repository has uncommitted changes, agent execution must be refused — not attempted on a dirty tree.
56. If a workspace directory already exists unexpectedly, execution must be refused — never silently delete or overwrite it.
